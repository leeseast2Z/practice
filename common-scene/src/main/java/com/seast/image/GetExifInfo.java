...
public class GetExifInfo {
    ...
    public static void main(String[] args) throws ImageProcessingException, IOException {
        String imgDir = "C:\\Users\\WIN10\\Desktop\\images";
    ...
    private static void printPicExifInfo(Map<String, String> map) throws IOException {
        String[] strings = new String[]{"Compression", "Image Width", "Image Height", "Make", "Model", "Software",
                ...
                "ISO Speed Ratings", "Exposure Time", "Exposure Mode", "F-Number", "Focal Length 35", "Color Space", "File Source", "Scene Type", "File Modified Date"};
        String[] names = new String[]{"压缩格式", "图像宽度", "图像高度", "拍摄手机", "型号", "手机系统版本号",
                "gps版本", "经度", "纬度", "高度", "UTC时间戳", "gps日期",
                "iso速率", "曝光时间", "曝光模式", "光圈值", "焦距", "图像色彩空间", "文件源", "场景类型", "修改日期"};
        for (int i = 0; i < strings.length; i++) {
            if (map.containsKey(strings[i])) {
                ...
                } else if("File Modified Date".equals(strings[i])) {
                    // DateUtils.parseDate(strings[i], "");
    ...
    private static void convertLatLng2Loaction(double gps_latitude, double gps_longitude) throws IOException {
        ...
        if (resp != null) {
            if (resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                res = IOUtils.readStreamAsString(entity.getContent(), CharEncoding.UTF_8);
